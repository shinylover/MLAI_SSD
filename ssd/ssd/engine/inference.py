...
def compute_on_dataset(backbone, boxhead, data_loader, device):
    results_dict = {}
...
def inference(backbone, boxhead, data_loader, dataset_name, device, output_folder=None, use_cached=False, **kwargs):
    ...
    if use_cached and os.path.exists(predictions_path):
        ...
        predictions = compute_on_dataset(backbone, boxhead, data_loader, device)
        synchronize()
...
@torch.no_grad()
def do_evaluation(cfg, backbone, boxhead, distributed, **kwargs):
    if isinstance(backbone, torch.nn.parallel.DistributedDataParallel):
        backbone = backbone.module
    if isinstance(boxhead, torch.nn.parallel.DistributedDataParallel):
        boxhead = boxhead.module
    backbone.eval()
    boxhead.eval()
    device = torch.device(cfg.MODEL.DEVICE)
    data_loaders_val = make_data_loader(cfg, is_train=False, distributed=distributed, drop_last=False)
    eval_results = []
    for dataset_name, data_loader in zip(cfg.DATASETS.TEST, data_loaders_val):
        output_folder = os.path.join(cfg.OUTPUT_DIR, "inference", dataset_name)
        if not os.path.exists(output_folder):
            mkdir(output_folder)
        eval_result = inference(backbone, boxhead, data_loader, dataset_name, device, output_folder, **kwargs)
